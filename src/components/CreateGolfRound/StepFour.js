...
class StepFour extends Component {
  componentDidMount() {
    ...
    if (!golfscores.length) {
      ...
      golfclubdata.forEach((hole, index) => {
        this.props.dispatch(
          arrayPush("createGolfRound", "golfholes", {
            number: hole.number,
            index: hole.index,
            par: hole.par
          })
        ...
        this.props.dispatch(
          arrayPush("createGolfRound", "golfscores", { hole: index + 1 })
        );
      });
    }
  }
              ...
              <List.Item key={index}>
                ...
                >
                  ...
                  <Grid style={{ fontSize: "20px", fontWeight: "bold" }}>
                    <Grid.Row columns={3}>
                      <Grid.Column>Hole: {holes[index].number}</Grid.Column>
                      <Grid.Column>Par: {holes[index].par}</Grid.Column>
                      <Grid.Column>Index: {holes[index].index}</Grid.Column>
                    </Grid.Row>
                  </Grid>
                  {/* First row */}
                </Segment>
                ...
                >
                  <Table.Row>
                    ...
                    <Table.Cell>Putts</Table.Cell>
                    <Table.Cell>Drive</Table.Cell>
                    <Table.Cell>Chips</Table.Cell>
                    <Table.Cell>Penalties</Table.Cell>
                  </Table.Row>
                  {/* Second Row */}
                  {players.map(player => (
                    <Table.Row>
  ...
  render() {
    ...
    return (
      <div>
        <Label ribbon size="huge" color="orange">
          How did it go?
        </Label>
        <FieldArray
          name="golfscores"
          component={this.renderStrokes}
          holes={holes}
          players={golfplayers}
        />
      </div>
